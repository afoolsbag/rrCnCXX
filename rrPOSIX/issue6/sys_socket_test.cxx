...
TEST(SysSocket, tcp)
{
    ...
    ASSERT_TRUE(!pthread_create(&thrd_server, NULL, tcp_server_func, NULL));

    pthread_t thrd_client;
    ASSERT_TRUE(!pthread_create(&thrd_client, NULL, tcp_client_func, NULL));

    ASSERT_TRUE(!pthread_join(thrd_server, NULL));
    ASSERT_TRUE(!pthread_join(thrd_client, NULL));
}
...
{
    pthread_t thrd_server;
    ASSERT_TRUE(!pthread_create(&thrd_server, NULL, udp_server_func, NULL));

    pthread_t thrd_client;
    ASSERT_TRUE(!pthread_create(&thrd_client, NULL, udp_client_func, NULL));
