...
#pragma once
#ifndef RRCXX_C2LINESEG_HXX_
#define RRCXX_C2LINESEG_HXX_
...
struct LineSegment {
    ...
    {
        this->ept1 = Point<ScalarType>(x1, y1);
        this->ept2 = Point<ScalarType>(x2, y2);
    }
...
inline bool Intersected(const Point<ScalarType> &p, const LineSegment<ScalarType> &s)
{
    ...
    // 若通过快速判定，则进行严格判定：
    const Vector<ScalarType> vAB = BoundVector<ScalarType>(s).vec;
    const Vector<ScalarType> vAP = BoundVector<ScalarType>(s.ept1, p).vec;
...
inline bool Intersected(const LineSegment<ScalarType> &s1, const LineSegment<ScalarType> &s2)
{
    ...
    // 若通过快速判定，则进行严格判定：
    const Vector<ScalarType> vAB = BoundVector<ScalarType>(s1);
    const Vector<ScalarType> vAC = BoundVector<ScalarType>(s1.ept1, s2.ept1);
    const Vector<ScalarType> vCD = BoundVector<ScalarType>(s2);
