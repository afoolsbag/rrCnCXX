...
#include "rrwindows/dtpui/strxc.h"
...
{
    CONST INT uRqdCnt = MultiByteToWideChar(inputCodePage, 0, inputString, inputBytesCount, NULL, 0);
    if (0 == uRqdCnt)
        return 0;
    ...
    CONST INT uCnt = MultiByteToWideChar(inputCodePage, 0, inputString, inputBytesCount, uStr, uRqdCnt);
    if (0 == uCnt) {
        HeapFree(GetProcessHeap(), 0, uStr);
        return 0;
    }

    CONST INT bCnt = WideCharToMultiByte(outputCodePage, 0, uStr, uCnt, outputString, outputBytesCount, NULL, NULL);
    HeapFree(GetProcessHeap(), 0, uStr);
    return bCnt;
}
