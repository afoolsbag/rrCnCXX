...
int _tmain(int argc, _TCHAR *argv[], _TCHAR *envp[])
{
        ...
        codeError = getaddrinfo(DEFAULT_ADDRESS, DEFAULT_PORT, &hintsAddrInfo, &pAddrInfo);
        if (codeError != NO_ERROR) {
        ...
        /* 尝试连接，直至有一例成功 */
        for (iAddrInfo = pAddrInfo; iAddrInfo != NULL; iAddrInfo = iAddrInfo->ai_next) {
                refSocket = socket(iAddrInfo->ai_family, iAddrInfo->ai_socktype, iAddrInfo->ai_protocol);
                ...
                codeError = connect(refSocket, iAddrInfo->ai_addr, iAddrInfo->ai_addrlen);
